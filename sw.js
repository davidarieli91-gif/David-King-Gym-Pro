...
// v146: c119 — header dumbbell grows + builder thumbnails grow (user request 2026-09-24): (1) PROGRAM BUILDER «added exercises» rows — the exercise thumbnail is 3× bigger (32→96px, the live-workout card size) and now ANIMATED with the same two-frame crossfade the live workout and the portal use (.ex-anim-container); the row header got flex-wrap so the big image + controls reflow instead of overflowing on narrow phones; (2) header dumbbell (trainer live-workout + client portal) — while a workout is running the button is twice as wide (2.25→4.5rem), the dumbbell icon grows 18→24px and the workout completion % (done sets / total sets, 1–100) is shown right next to it inside the same button; trainer % recomputes on every ✓/✕ via updateProgressBar + dkDumbbellPctSync, portal re-syncs every second with the session; zero mechanic changes — same click targets, same return-to-workout behavior
...
const CACHE_NAME = 'dk-gym-v146'; // c119
